...
#End Region
    ...
    Public Overrides Sub RenderControl(writer As HtmlTextWriter)
        ...
        Dim ctlHasEducationHealthCarePlanUDF As DropDownList = TryCast(fldStudentDetailUserDefinedHealth13.InternalFieldControl, DropDownList)
        Dim hasEHCPYes As ListItem = New ListItem("Yes", "3")
        Dim hasEHCPNo As ListItem = New ListItem("No", "4")
        'Dim hasEHCPYesNo As New ListItemCollection
        ctlHasEducationHealthCarePlanUDF.Items.Add(hasEHCPYes)
        ctlHasEducationHealthCarePlanUDF.Items.Add(hasEHCPNo)



        'ctlHasEducationHealthCarePlanUDF.DataSource = hasEHCPYesNo
        'ctlHasEducationHealthCarePlanUDF.DataBind()
        'ctlHasEducationHealthCarePlan.Items.Add(hasEHCPNo)
        ...
        Dim ctlPrior As DropDownList = TryCast(fldPrior.InternalFieldControl, DropDownList)
        Dim ctlPriorList As New List(Of ListItem)

        For Each item As ListItem In ctlPrior.Items
            ctlPriorList.Add(item)
        Next

        Dim ctlPriorListOrdered = ctlPriorList.OrderBy(Function(item) item.Value)

        ctlPrior.DataSource = ctlPriorListOrdered
        ctlPrior.DataBind()

        ' Dim ctlLanguage As DropDownList = TryCast(fldLanguage.InternalFieldControl, DropDownList)


        'Dim item01 = ctlPrior.Items.FindByValue("01")

        'Dim item02 = ctlPrior.Items.FindByValue("02")

        'Dim item03 = ctlPrior.Items.FindByValue("03")

        'Dim item04 = ctlPrior.Items.FindByValue("04")

        'Dim item05 = ctlPrior.Items.FindByValue("05")

        'Dim item06 = ctlPrior.Items.FindByValue("06")

        'Dim item07 = ctlPrior.Items.FindByValue("07")

        'Dim item08 = ctlPrior.Items.FindByValue("08")

        'Dim item09 = ctlPrior.Items.FindByValue("09")

        'Dim itemP10 = ctlPrior.Items.FindByValue("10")

        'Dim item97 = ctlPrior.Items.FindByValue("97")

        'Dim item98 = ctlPrior.Items.FindByValue("98")

        'Dim item99 = ctlPrior.Items.FindByValue("99")

        'ctlPrior.Items.Remove(itemP10)
        'ctlPrior.Items.Insert(1, itemP10)


        'ctlPrior.Items.Remove(item09)
        'ctlPrior.Items.Insert(2, item09)

        'ctlPrior.Items.Remove(item08)
        'ctlPrior.Items.Insert(3, item08)

        'ctlPrior.Items.Remove(item07)
        'ctlPrior.Items.Insert(4, item07)

        'ctlPrior.Items.Remove(item06)
        'ctlPrior.Items.Insert(5, item06)

        'ctlPrior.Items.Remove(item05)
        'ctlPrior.Items.Insert(6, item05)

        'ctlPrior.Items.Remove(item04)
        'ctlPrior.Items.Insert(7, item04)

        'ctlPrior.Items.Remove(item03)
        'ctlPrior.Items.Insert(8, item03)

        'ctlPrior.Items.Remove(item02)
        'ctlPrior.Items.Insert(9, item02)

        'ctlPrior.Items.Remove(item01)
        'ctlPrior.Items.Insert(10, item01)
    ...
    Private Sub CheckData()
        ...
        '    WorkingData.EnrolmentRequestRow.DateOfEntryUK = CStr(DOEUK.Value)
        'End If
        ...
        'WorkingData.EnrolmentRequestRow.AcceptMarketingConsent = 1
        'WorkingData.EnrolmentRequestRow.AcceptShareInfoConsent = 1
    ...
    Protected Sub btnContinue_click(ByVal sender As Object, ByVal e As EventArgs) Handles btnContinue.Click
        ...
        WorkingData.EnrolmentRequestRow.EuroResidentID = Nothing
